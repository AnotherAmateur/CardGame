...
public partial class GFieldController : Node2D, ISocketConn
{
	...
	public string GetCurState()
	{
		...
		string rowsTotalsPl1 = string.Join("-", (protagonist.TotalsByRows.Values).Select(x => x / 4));
		curState.Add(rowsTotalsPl1);

		string rowsTotalsBot = string.Join("-", (antagonist.TotalsByRows.Values).Select(x => x / 4));
		...
		string handWeight = (aiPlayer.Hand.Select(x => x.Strength).Sum() / 3).ToString();
